...
- 📱 **App teilen via QR-Code** — Im User-Dropdown öffnet „App teilen" einen Dialog mit scanbarem QR-Code, kopierbarer URL und Web-Share-API-Button (Mobile). Auf einer Rätsel-Seite wird automatisch der puzzle-spezifische Share-Link genutzt
- 🖼️ **Per-Puzzle Social Previews** — geteilte Rätsel-Links zeigen auf WhatsApp/Twitter/iMessage/Slack ein **custom-generiertes 1200×630-Thumbnail** mit Titel, Theme-Chip, Schwierigkeits-Chip und Mini-Grid (gerendert via satori + @resvg/resvg-js zur Build-Zeit, Schema.org-Game-JSON-LD pro Puzzle)
